<div id="add-product-modal" class="pt-12 hidden fixed top-0 right-0 left-0 z-50 flex justify-center items-center w-full h-screen bg-black bg-opacity-50 backdrop-blur-sm">
  <div class="relative p-4 w-full max-w-md max-h-full">
    <div class="bg-white rounded-lg shadow-sm overflow-y-auto max-h-[80vh]">
      ...
      <form action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data" class="p-4">
        @csrf
        <div class="grid gap-4 mb-4">
          ...
          <div class="col-span-2">
            ...
            <textarea name="description" id="description" required class="w-full rounded-lg p-2.5 bg-gray-50" rows="4"></textarea>
          </div>

          <div class="col-span-2">
            <label for="purchase_price" class="block text-sm font-medium text-gray-700">Harga Beli</label>
            <input type="number" name="purchase_price" id="purchase_price" required class="w-full rounded-lg p-2.5 bg-gray-50" min="0">
          </div>

          <div class="col-span-2">
            <label for="selling_price" class="block text-sm font-medium text-gray-700">Harga Jual</label>
            <input type="number" name="selling_price" id="selling_price" required class="w-full rounded-lg p-2.5 bg-gray-50" min="0">
          </div>

          <div class="col-span-2">
            <label for="stock" class="block text-sm font-medium text-gray-700">Stok Produk</label>
            <input type="number" name="stock" id="stock" required class="w-full rounded-lg p-2.5 bg-gray-50" min="0">
          </div>

          <div class="col-span-2">
            <label for="low_stock_threshold" class="block text-sm font-medium text-gray-700">Batas Stok Rendah</label>
            <input type="number" name="low_stock_threshold" id="low_stock_threshold" required class="w-full rounded-lg p-2.5 bg-gray-50" min="0">
          </div>

          <div class="col-span-2">
            <label for="expiry_date" class="block text-sm font-medium text-gray-700">Tanggal Kedaluwarsa</label>
            <input type="date" name="expiry_date" id="expiry_date" class="w-full rounded-lg p-2.5 bg-gray-50">
          </div>
          ...
          <div id="image-preview" class="col-span-2 mt-4">
            ...
            <img id="preview" class="w-48 h-48 object-cover rounded-md" src="" alt="Image Preview" style="display: none;">
          </div>
        </div>
...
<script>
  ...
  document.querySelector('[data-modal-toggle="add-product-modal"]').addEventListener('click', function () {
    const preview = document.getElementById('preview');
    preview.src = '';  
